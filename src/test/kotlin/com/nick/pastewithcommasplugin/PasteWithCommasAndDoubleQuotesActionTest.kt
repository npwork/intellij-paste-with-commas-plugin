...
class PasteWithCommasAndDoubleQuotesActionTest {

    private val action = PasteWithCommasAndDoubleQuotesAction()

    @Test
    fun `test single line input gets double quoted`() {
        val input = "single line"
        val expected = "\"single line\""
        val result = action.addCommasAndDoubleQuotesToLines(input)
        assertEquals(expected, result)
    }

    @Test
    fun `test two lines gets double quotes and comma`() {
        val input = "line1\nline2"
        val expected = "\"line1\",\n\"line2\""
        val result = action.addCommasAndDoubleQuotesToLines(input)
        assertEquals(expected, result)
    ...
    @Test
    fun `test lines with existing double quotes`() {
        val input = "line1\"\nline2\nline3"
        val expected = "\"line1\"\",\n\"line2\",\n\"line3\""
        val result = action.addCommasAndDoubleQuotesToLines(input)
        assertEquals(expected, result)
    ...
    @Test
    fun `test SQL-like columns example`() {
        val input = "id\nname\ncreated_at\nupdated_at"
        val expected = "\"id\",\n\"name\",\n\"created_at\",\n\"updated_at\""
        val result = action.addCommasAndDoubleQuotesToLines(input)
        assertEquals(expected, result)
